...
class Solution:
    def groupThePeople(self, group_sizes: list[int]) -> list[list[int]]:
        grouped_by_group_size = defaultdict(list)
        result = []
        for person_id, group_size in enumerate(group_sizes):
            grouped_by_group_size[group_size].append(person_id)
            if len(grouped_by_group_size[group_size]) == group_size:
                result.append(grouped_by_group_size.pop(group_size))
        return result
